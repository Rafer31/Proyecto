...
export class ReservaService {
  ...
  async verificarReservaExistente(
    ...
    if (!isStaff) {
      ...
      if (reservaExistente) {
        const esMismoViaje =
          reservaExistente.idplanificacion === idplanificacionActual;
        return {
          tieneReserva: true,
    ...
    if (isRetorno) {
      ...
      if (retornoInfo) {
        ...
        const iddestinoIda = Array.isArray(viajeIda)
          ? viajeIda[0]?.iddestino
          : viajeIda?.iddestino;
      ...
      if (todasReservas && todasReservas.length > 0) {
        ...
        const registroCanceladoMismoDestino = viajesExistentes?.find(
          (v) =>
            ...
            todasReservas.find(
              (r) =>
                r.idplanificacion === v.idplanificacion &&
                r.estado === 'cancelado'
            )
        );
        ...
        if (registroCanceladoMismoDestino) {
          await this.supabase
            ...
            .eq(
              'idplanificacion',
              registroCanceladoMismoDestino.idplanificacion
            );
        } else {
          await this.supabase
            .from('asignaciondestino_planificacionviaje')
  ...
  private async programarNotificacionReserva(
    ...
    try {
      ...
      if (error || !viaje) {
        console.error('❌ Error obteniendo viaje para notificación:', error);
        return;
      }
      ...
      if (viaje.destino) {
        if (Array.isArray(viaje.destino) && viaje.destino.length > 0) {
          nombreDestino = (viaje.destino[0] as any)?.nomdestino || 'su destino';
        } else {
          nombreDestino = (viaje.destino as any)?.nomdestino || 'su destino';
        }
      }
  ...
  async cambiarReserva(
    ...
    const viajesAntiguos = new Set(
      reservasActuales.map((r) => r.idplanificacion)
    );
